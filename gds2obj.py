#!/usr/bin/env python3
"""
This program converts a GDSII (2D) layout file to multiple OBJ (3D) files (one per layer)

USAGE:
    - edit the "layerstack" variable in the "CONFIGURATION" section below
    - run "gdsiiobj file.gds"
OUTPUT:
    - one file per layer named: file.gds_layername.gltf
...
for cell in gdsii.top_level(): # loop through cells to read paths and polygons
    ...
    for layer_number, polygons in layers.items():
        ...
        output_file.write("o " + object_name + "\n")
        output_file.write("\n# Vertices\n")
        np.savetxt(output_file ,  positions, delimiter=" ", fmt='v %1.4e %1.4e %1.4e')

        output_file.write("\n# Faces\n")
        # add 1 tode indices as OBJ file format starts counting from 1 instead of 0
        np.savetxt(output_file ,  indices + 1, delimiter=" ", fmt='f %d %d %d')
